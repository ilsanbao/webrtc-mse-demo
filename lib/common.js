...
var ipip = require('ipip')({data: __dirname + '/../data/mydata4vipday2.datx', watch: false});

module.exports = {
    geoHash: function (lat, lon) {
        return {
            level1: ngeohash.encode_int(lat, lon, 11),
            level2: ngeohash.encode_int(lat, lon, 20)
        };
    },
    getClientIp: function (socket) {
        var headers = socket.handshake.headers;
        var ip = headers["x-real-ip"] || headers['remote_addr'] || headers['http_x_forwarded_for'] && headers['http_x_forwarded_for'].split(',')[0] || socket.request.connection.remoteAddress || socket.handshake.address;
        return ip.replace(/^.*:/, '');
    },
    ...
    clientGeoHash: function (socket) {
        var ip = this.getClientIp(socket);
        if (this.isValidIpv4(ip)) {
            var ipInfo = ipip(ip);
            return this.geoHash(ipInfo.lantitude, ipInfo.lontitude);
        }
    }
}
